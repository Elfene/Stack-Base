...
export async function GET() {
  try {
    ...
    for (let i = 0; i < raw.length; i += 2) {
      const data = typeof raw[i] === 'string' ? JSON.parse(raw[i] as string) : raw[i];
      entries.push({
        player: data.player,
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await redis.zscore(LEADERBOARD_KEY, JSON.stringify({ player, timestamp: 0 }));
    ...
    for (let i = 0; i < allMembers.length; i += 2) {
      const data = typeof allMembers[i] === 'string' ? JSON.parse(allMembers[i] as string) : allMembers[i];
